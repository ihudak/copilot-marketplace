...
model_routing:            # optional; if present, echo back in output.
  classification: SIGNIFICANT      # See _shared/model-routing.md §4 for full schema.
  # The orchestrator MUST re-invoke this sub-agent under Opus for HIGH-RISK
  # CVEs, and SHOULD re-invoke for SIGNIFICANT CVEs involving a major bump
  # or non-trivial breaking-change surface (per fix-vuln/SKILL.md Step 0).
