---
name: api-guideline-review
description: Review OpenAPI specification files against Dynatrace REST API and IAM permission naming guidelines. Use when reviewing API specs for compliance, validating OpenAPI documents against Dynatrace standards, checking IAM scope naming, or auditing API designs. Triggers on requests to review OpenAPI/Swagger specs, check API compliance, validate Dynatrace API guidelines, or review IAM permissions.
---
...
Load ALL guideline files before reviewing - never use a subset:

**REST API Guidelines** (in `references/rest-api-guidelines/`):
...
**Permission Guidelines** (in `references/permission-guidelines/`):
- `Introduction.md` - IAM permission format `{service}:{resource}:{action}`
- `General Mapping.md` - URL-to-IAM mapping rules

**Template**: `references/template/openapi-template.yaml`
...
- **Location**: File and line reference (e.g., `openapi.yaml line 42`)
- **Current**: Code snippet showing the issue
- **Fix**: Exact code to resolve

## Potential Improvements
Deviations from SHOULD/SHOULD NOT recommendations.

Same format as Mistakes.
...
**Mistakes (MUST violations)**:
- Missing required elements (security, headers)
- Version inconsistency
- Using `allOf` in schemas
- Proprietary HTTP status codes (9xx)
- Missing operationId
- Incorrect IAM scope format

**Improvements (SHOULD violations)**:
- Naming convention deviations
- Missing recommended response codes
- Suboptimal pagination approach
