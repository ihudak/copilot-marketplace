...
# impl:jira:docs:, impl:jira:epics:, vuln:, upgrade: (and /slash variants)
# and routes per spec §3:
...
#   • impl:                               → silent (dispatcher / help-only)
...
# Require at least one non-whitespace, non-flag argument so bare prefixes
# don't inject noise on every misfire. The first capture group holds the full
# command token (e.g. "impl", "impl:code", "impl:jira:docs").
# Match both prefix form (impl:code: ...) and slash form (/impl:code ...)
if [[ "$prompt" =~ ^/?(impl(:(code|docs|jira(:(docs|epics))?))?|vuln|upgrade):?[[:space:]]+[^[:space:]-] ]]; then
    cmd="${BASH_REMATCH[1]}"
elif [[ "$prompt" =~ ^/(impl(:(code|docs|jira(:(docs|epics))?))?|vuln|upgrade)[[:space:]]+[^[:space:]-] ]]; then
    cmd="${BASH_REMATCH[1]}"
else
    exit 0
fi
...
case "$cmd" in
    ...
    impl:jira*)
        ...
        # (handled identically since any path into the :jira branch needs
